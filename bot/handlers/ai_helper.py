...
@router.message(F.text.in_(["🤖 AI Yordamchi", "🤖 AI Помощник", "🤖 AI Helper"]))
async def ai_helper_start(message: Message, state: FSMContext):
    """Start AI helper conversation."""
...
async def ai_helper_message(message: Message, state: FSMContext):
    """Handle AI helper messages."""
    lang = await db.get_user_language(message.from_user.id)
    
    # Check for cancel
    if message.text and message.text.lower() in ["bekor", "отмена", "cancel", "❌ bekor qilish", "❌ отмена", "❌ cancel"]:
        await state.clear()
        await message.answer(
